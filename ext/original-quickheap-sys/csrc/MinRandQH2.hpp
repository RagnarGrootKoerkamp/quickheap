 ...
 *  circular array version, optimized insertion
 *
 */

#ifndef SQH_MIN_RAND_QH2_HPP
#define SQH_MIN_RAND_QH2_HPP
...
template<class T> class MinRandQH2 {
protected:
...
template<class T> MinRandQH2<T>::MinRandQH2(int size, bool randomSeed) :
...
template<class T> inline MinRandQH2<T>::~MinRandQH2() {
  delete []heap;
}

template<class T> string MinRandQH2<T>::toString() const { 
  ostringstream oss;
...
template<class T> ostream& operator<< (ostream &os, const MinRandQH2<T> &rhs) {
  os << rhs.toString();
  return os;
}

template<class T> inline void MinRandQH2<T>::swap(int pi, int pj) {
...
template<class T> inline int MinRandQH2<T>::size() const {
  return stack.stack[0] - idx;
}

template<class T> inline bool MinRandQH2<T>::isEmpty() const {
  return (stack.stack[0] == idx);
}

template<class T> inline void MinRandQH2<T>::clear() {
...
template<class T> bool MinRandQH2<T>::insert(const T &item) {
...
template<class T> int MinRandQH2<T>::partition(int pj) {
...
template<class T> T MinRandQH2<T>::findMin() {
...
template<class T> T MinRandQH2<T>::extractMin() {
  T item = findMin();
...
template<class T> bool MinRandQH2<T>::deleteByPos(int pos) {
...
template<class T> inline T MinRandQH2<T>::getByPos(int pos) const {
  //if ( (pos < idx) || (pos >= stack.stack[0]) ) return false; // error!, out of boundaries
