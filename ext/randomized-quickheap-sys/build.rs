...
fn main() {
    ...
    build
        ...
        .flag("-march=native")
        .flag("-flto")
        .flag("-fpermissive") // MinRandQH2.hpp uses template<T> (non-type) in friend decl
        .warnings(false)
    ...
    println!("cargo:rerun-if-changed=csrc/MinRandQH2.hpp");
    println!("cargo:rerun-if-changed=csrc/Stack.hpp");
}
